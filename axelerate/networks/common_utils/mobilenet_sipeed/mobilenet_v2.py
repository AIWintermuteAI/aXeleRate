...
def MobileNetV2(input_shape=None,
    ...
    if input_shape is not None and input_tensor is not None:
        try:
            ...
            try:
                is_input_t_tensor = backend.is_keras_tensor(keras_utils.get_source_inputs(input_tensor))
            except ValueError:
                raise ValueError('input_tensor: ', input_tensor, 'is not type input_tensor')
    ...
    x = layers.Conv2D(32, kernel_size=3, strides=(2, 2), padding='valid', use_bias=False, name='Conv1')(x)
    ...
    x = _inverted_res_block(x,
                            ...
                            expansion=6,
                            block_id=1,
                            expand_channel=48 if alpha > .6 else None)
    x = _inverted_res_block(x,
                            filters=24,
                            alpha=alpha,
                            stride=1,
                            expansion=6,
                            block_id=2,
                            expand_channel=124 if alpha > .6 else None)
    ...
    # Create model.
    model = models.Model(inputs, x, name='mobilenetv2_%0.2f_%s' % (alpha, rows))  # type:tf.keras.Model
    ...
    if weights == 'imagenet':
        if include_top:
            ...
            model_name = ('mobilenet_v2_weights_tf_dim_ordering_tf_kernels_' + str(alpha) + '_' + str(rows) + '_no_top' +
                          '.h5')
            weight_path = BASE_WEIGHT_PATH + model_name
            weights_path = keras_utils.get_file(model_name, weight_path, cache_subdir='models')
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id, expand_channel=None, pointwise_channel=None):
    channel_axis = 1 if backend.image_data_format() == 'channels_first' else -1
    ...
    if stride == 2:
        ...
        x = layers.ZeroPadding2D(padding=[[1, 1], [1, 1]], name=prefix + 'pad')(x)
